    ...
    if not ray.is_initialized():
        ...
        ray.init(
            runtime_env={"env_vars": {"TOKENIZERS_PARALLELISM": "true", "NCCL_DEBUG": "WARN", "VLLM_LOGGING_LEVEL": "WARN",
             ...
             "VLLM_TORCH_PROFILER_DIR":"/cpfs01/user/zhoujiecheng/workload_rl_analyse/search-r1/profile/rollout_trace/search-r1-7b"}},
...
def _parse_workload_step_groups(value: str):
    groups = []
    for group in value.split(";"):
        steps = [item.strip() for item in group.split(",") if item.strip()]
        if steps:
            groups.append(steps)
    return groups or [["0"]]


def create_benchmark_dataset(data_config, tokenizer):
    import torch
    from torch.utils.data import Dataset

    class BenchmarkDataset(Dataset):
        def __init__(self):
            self.length = max(int(data_config.get("train_batch_size", 1)), int(data_config.get("val_batch_size", 1) or 1), 1)
        ...
        def __getitem__(self, item):
            input_ids = torch.full((self.max_prompt_length,), int(self.pad_token_id), dtype=torch.long)
            ...
            return {
                ...
                "extra_info": {"index": int(item), "split": "benchmark"},
                "index": int(item),
            }
...
class TaskRunner:
    def run(self, config):
        ...
        benchmark_mode = _env_flag("WORKLOAD_BENCHMARK", "0")
        if benchmark_mode:
            train_dataset = create_benchmark_dataset(config.data, tokenizer)
            val_dataset = create_benchmark_dataset(config.data, tokenizer)
        else:
        ...
        if benchmark_mode:
            ...
            default_workload_path = os.path.join(
                os.getenv("RAY_DATA_HOME", "/mnt/shared-storage-user/ailab-sys/zhoujiecheng/aliyun_data/search_r1"),
                "profile/multiturn_workload_log",
            )
            workload_path = os.getenv("WORKLOAD_PATH") or os.getenv("WORKLOAD_LENGTH_DIR") or default_workload_path
            step_groups = _parse_workload_step_groups(os.getenv("WORKLOAD_STEP_GROUPS", "0"))
            generator = Generator(
                workload_path,
                local_path,
            ...
            print(f"Benchmark workload path: {workload_path}, step groups: {step_groups}")
            for repeat_idx in range(benchmark_repeat):
                print(f"Benchmark repeat {repeat_idx + 1}/{benchmark_repeat}")
                for group_idx, steps in enumerate(step_groups):
                    for step in steps:
                        print(
                            f"Benchmark repeat {repeat_idx + 1}/{benchmark_repeat}, "
                            f"group {group_idx + 1}/{len(step_groups)}, step: {step}"
                        )
