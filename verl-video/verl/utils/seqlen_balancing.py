...
def rearrange_micro_batches(batch: TensorDict, max_token_len, dp_group=None, same_micro_num_in_dp=True, min_num_micro_batch=None):
    """
    ...
    for partition in micro_bsz_idx:
        ...
        if curr_micro_batch_list:
            concatenated_micro_batch = torch.cat(curr_micro_batch_list)
            micro_batches_tds.append(concatenated_micro_batch)
        elif num_micro_batches == 1 and not partition:
            micro_batches_tds.append(TensorDict({}, batch_size=[]))

    return micro_batches_tds, micro_bsz_idx
