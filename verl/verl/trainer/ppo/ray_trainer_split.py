...
class RayPPOTrainer:
    ...
    def init_workers(self):
        ...
        if self.hybrid_engine:
            ...
            # Get the resource pool for Rollout
        ...
        for resource_pool, class_dict in self.resource_pool_to_cls.items():
            ...
            wg_dict = self.ray_worker_group_cls(resource_pool=resource_pool, ray_cls_with_init=worker_dict_cls, **wg_kwargs) # 声明 ray worker group
            spawn_wg = wg_dict.spawn(prefix_set=class_dict.keys()) #创建 worker
    ...
    def _save_checkpoint(self):
        ...
        save_actor_cls.save_checkpoint(actor_local_path, actor_remote_path, self.global_steps, max_ckpt_to_keep=max_actor_ckpt_to_keep)
    ...
    def _load_checkpoint(self):
        ...
        load_actor_cls.load_checkpoint(actor_path, del_local_after_load=self.config.trainer.del_local_ckpt_after_load)
        # load critic
        if self.use_critic:
    ...
    def _balance_batch(self, batch: DataProto, metrics, logging_prefix="global_seqlen"):
        ...
        if self.hybrid_engine:
            # actor_rollout_wg has world_size
            world_size = self.actor_rollout_wg.world_size
        else:
            # actor_wg has world_size
