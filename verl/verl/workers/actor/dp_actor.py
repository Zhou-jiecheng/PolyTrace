            ...
            for batch_idx, data in enumerate(dataloader):
                ...
                if self.do_profile:
                    if self.profiler is not None:
                        self.profiler.start()  
                # log_path = "/cpfs01/user/zhoujiecheng/workload_rl_analyse/verl/profile/print.log"

                # with open(log_path, "a") as f:  # "a" 表示追加写入
                #     f.write(f"batch_idx={batch_idx}\n")
                #     f.write(f"do_profile={self.do_profile}\n")
                mini_batch = data
                if has_multi_modal_inputs:
                ...
                if self.do_profile and self.profiler is not None:
                    if batch_idx == 0:
                        self.profiler.stop()
                        torch.cuda.memory._dump_snapshot("/cpfs01/user/zhoujiecheng/workload_rl_analyse/verl/profile/memory_trace/DAPO_7B_KVC0p4.pickle")
                        self.do_profile=False

            append_to_dict(metrics, data)
        self.actor_optimizer.zero_grad()
        if self.count % 100 == 0:
            # re init the profiler
            self.do_profile=True
        return metrics
