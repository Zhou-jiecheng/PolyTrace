...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        self.if_profile = int(os.getenv("VERL_PROFILE", 0))
        self.step = 0
        ...
        self.workload_length_dir = os.getenv(
            "WORKLOAD_LENGTH_DIR",
            "/mnt/shared-storage-user/ailab-sys/zhoujiecheng/aliyun_data/verl/profile/packed_length_log",
        )
    ...
    def _enforce_response_token_length(self, token_ids, target_len, eos_token_id):
        ...
        eos_token_ids = {int(token_id) for token_id in _as_int_list(eos_token_id)}
        eos_token_id = next(iter(eos_token_ids), int(self.pad_token_id))
        filler_token_id = 0
        while filler_token_id in eos_token_ids or filler_token_id == int(self.pad_token_id):
            filler_token_id += 1

        token_ids = [int(token_id) for token_id in token_ids[:target_len]]
        token_ids = [filler_token_id if token_id in eos_token_ids else token_id for token_id in token_ids]
        if len(token_ids) < target_len:
            token_ids.extend([filler_token_id] * (target_len - len(token_ids)))
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        input_seqlen_lst = prompt_lengths
        ...
        if self.if_profile == 1 and self.do_profile:
            ...
            for output_idx, output in enumerate(outputs):
                for sample_id in range(len(output.outputs)):
                    ...
                    if workload_output_groups is not None:
                        ...
                        token_ids = self._enforce_response_token_length(token_ids, target_len, eos_token_id)
                    response.append(token_ids)

            response = pad_2d_list_to_length(response, self.pad_token_id, max_length=self.config.response_length).to(idx.device)

            if use_workload_output_len or (self.sampling_params.n > 1 and do_sample):
                repeat_times = workload_rollout_n if use_workload_output_len else self.sampling_params.n
                ...
                for key, value in list(non_tensor_batch.items()):
                    try:
                        ...
                        continue
                    if value_len == prompt_batch_size:
                        non_tensor_batch[key] = _repeat_interleave(value, repeat_times)
        ...
        if self.collect_workload_lengths:
            ...
            output_length_groups = [
                output_seqlen_lst[i * outputs_per_prompt : (i + 1) * outputs_per_prompt]
        ...
        if not is_validate or self.collect_validate_workload_lengths:
            self.step += 1
